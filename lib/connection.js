...
IceConnection.prototype.disconnect = function() {
    ...
    debug('disconnected');
    if (room) {
        // release the reference to this connection
        room.connections[this.id] = undefined;
    ...
    // release the token from storage
    if (this.token) {
        this.server.storage.releaseToken(this.token);
    }
...
IceConnection.prototype.messagePreAuth = function(message, callback) {
    ...
    // reset room specific connection information
    this.token = undefined;
    this.room = undefined;
    this.id = -1;
    ...
    this.server.storage.findToken(token, function(err, tokenData, room) {
        ...
        // initialise the next connection id
        nextId = room.connections.indexOf(undefined);

        // update the token data
        connection.token = token;
        connection.room = room;

        // allocate the connection the first unused client slot
        connection.id = nextId >= 0 ? nextId : room.connections.length;

        // save the connection in the room clients
